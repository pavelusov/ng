...
export function ServiceLeadChatPanel({ serviceLeadId, title = "Чат", subtitle }: Props) {
  const { data: session } = useSession();
  const currentUserId = session?.user?.id ?? "";

  const { socket, socketConnected, setOpenConversationId, joinConversationRoom, clearUnreadForLead, refreshUnreadByLeads } =
    useChatSocket();
  ...
  }, [serviceLeadId, loadMessages, markRead, setOpenConversationId]);
