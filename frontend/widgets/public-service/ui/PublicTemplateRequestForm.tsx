...
export function PublicTemplateRequestForm({ templateId, isAuthenticated }: Props) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!isAuthenticated) {
        router.push(buildServiceRequestAuthHref("signup", { kind: "TEMPLATE", templateId }));
      } else {
  ...
  return (
    <Stack spacing={2} component="form" onSubmit={handleSubmit}>
      ...
      <Button type="submit" variant="contained" size="large" disabled={busy || Boolean(validationError)} sx={{ fontWeight: 800, textTransform: "none", px: 3, py: 1.25 }}>
