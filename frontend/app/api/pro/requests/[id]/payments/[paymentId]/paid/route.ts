...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string; paymentId: string }> }) {
  ...
  try {
    ...
    const response = await fetchBackendAsUser(`/pro/requests/${id}/payments/${paymentId}/paid`, session.user.id, { method: "POST" });
    const payload = await response.json().catch(() => ({ error: "Failed to mark paid" }));
    return NextResponse.json(payload, { status: response.status });
  } catch (error) {
    console.error("Error marking pro payment as paid:", error);
