...
describe("GET /api/profile/leads", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it("returns customer leads for authenticated user", async () => {
    const payload = [{ id: "lead-1", status: "NEW" }];
    ...
    expect(mockedFetchBackendAsUser).toHaveBeenCalledWith("/service-leads/mine", "user-1");
  });
});
