...
export async function GET() {
  try {
    ...
    const response = await fetchBackendAsUser("/contracts/legal-profile", session.user.id);
    const payload = await response.json().catch(() => ({ error: "Failed to fetch legal profile" }));
    return NextResponse.json(payload, { status: response.status });
  } catch (error) {
    console.error("Error fetching customer legal profile:", error);
...
export async function PUT(request: Request) {
  try {
    ...
    console.error("Error saving customer legal profile:", error);
